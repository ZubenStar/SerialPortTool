...
- **检查更新 / 自动更新**：每次启动都会在后台静默检查 GitHub Releases（程序持续运行时每 24 小时再补查一次），有新版本时提示并可下载安装；「帮助 → 检查更新」可随时手动检查。
...
- **启动静默检查**：每次启动后数秒都会在后台检查一次，只有发现新版本才会提示。**不存在**「距上次检查不足 24 小时就不查」的抑制，因此刚发布的版本不会因为缓存而被推迟发现。
- **运行期检查**：程序持续运行时每 24 小时自动补查一次，长时间挂机也能收到新版本提示。
- **检查失败退避**：断网、请求超时或被 GitHub 限流后的 1 小时内不再重试（避免请求风暴）；网络恢复后最多 1 小时即可重新发现更新。
- **手动检查**：「帮助 → 检查更新」随时触发，始终立即联网（不受退避影响），无论结果如何都会给出明确反馈；手动检查也不会改变后续静默检查的节奏。
