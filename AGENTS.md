...
Added in v2.2.2 — the silent check now stops re-asking once it knows the answer, and is throttled on the **success** side while it does not. Both changes come from the same report: the "a successful check leaves no trace, so every launch checks once" rule (v2.1.2) turned into real 403s, because an every-launch check is one request per launch and a machine that merely restarts the app a few dozen times an hour (`F5` / `dotnet run` / the auto-update relaunch), or that shares its public IP with other GitHub users behind NAT or a proxy, hits the anonymous 60/hour cap. The symptom is `更新检查过于频繁（GitHub 接口限流）` — it shows up on the manual path first, and then on every path. A dismissed update (稍后) must still re-prompt on the very next launch, which is why it is the *cache*, not a longer window, that carries that case:

- **rapid relaunches cost nothing**: launch the build three times back to back. Only the first may hit the network — count requests with `GET https://api.github.com/rate_limit` (reading it does not consume quota). The 2nd and 3rd must log `Skipped silent update check: the previous attempt succeeded less than 30min ago` at `Debug`, issue no request, and leave `Update.SilentSuccessUtc` in `settings.json` unchanged.
- **the cooldown does expire**: hand-edit `Update.SilentSuccessUtc` to a timestamp 31 minutes in the past (or temporarily lower `SilentSuccessCooldownMinutes`), then relaunch — the request must go out and the key must be rewritten.
- **a new Release is still found promptly**: with a cooldown in force, Help → Check for updates must report the new version immediately, and must leave `Update.SilentSuccessUtc` and `Update.SilentFailureUtc` untouched. With a Release newer than the running build, the app log shows `Update available: …` on the first launch — not on every launch any more.
- **a known update is never re-requested**: with a Release newer than the running build, launch once and dismiss the dialog with **稍后**. `Update.PendingRelease` must now be in `settings.json`. Relaunch — the dialog must come back immediately (no waiting out the cooldown), the log must show `Update check: reusing the cached pending update …` at `Debug` rather than `Update available: …`, and the `rate_limit` request count must **not** move. Repeat after more than 30 minutes: still no request, still the dialog.
- **the cache expires after a day**: with `Update.PendingRelease` in place, hand-edit `Update.PendingReleaseUtc` to 25 hours in the past and relaunch. Exactly one request must go out (log: `the cached pending update … is over 24h old; verifying against GitHub` at `Debug`, then `Update available: …`); if that version is still the latest, both keys must be present again with a fresh timestamp, so the following launch is silent again. Repeat with the timestamp key **deleted** entirely — a cache of unknown age must be treated as expired, not trusted.
- **skipping drops the cache**: in that same state, relaunch and click 跳过此版本. `Update.PendingRelease` must be gone, and the next launch must hit the network once and log nothing about a cached pending update. Also confirm `Update.SkippedVersion` holds that version.
- **installing drops the cache**: with `Update.PendingRelease` present and pointing at the version you are about to install, run the update to completion. The relaunched process must not show the dialog again and must log `Update available: …` only if an even newer Release exists — a stale cache must never pin the app to the version it just installed.
- **a manual check refreshes the cache**: delete `Update.PendingRelease`, then Help → Check for updates against a newer Release. The key must be written again (and the two throttle keys must be untouched), so the next launch finds the update without a request.
- **failure beats the cooldown**: go offline and launch twice (with no pending update cached). Even right after a successful check (no failure timestamp present) the first offline launch must write `Update.SilentFailureUtc` (never a success timestamp), and the second launch within the hour must skip with no request. Restore the network and confirm the next success (after the cooldown) deletes the key.
- **throttled is not silenced**: inside the cooldown and with no cache, a silent check must raise no dialog, and the manual path must still show 已是最新 / 发现新版本 as usual.
- **a corrupt cache fails open**: replace the value of `Update.PendingRelease` with `{` (or `"x"`), relaunch — no crash, no dialog from the cache, and the normal network check runs as if there were no cache at all.
- **legacy key stays gone**: `Update.LastCheckUtc` must be gone from `settings.json` after the first launch of this build, and must not come back.
...
| `IUpdateService` / `UpdateService` | Checks `api.github.com/repos/ZubenStar/SerialPortTool/releases/latest` for a newer version, compares versions **numerically** via `Version.TryParse`, and owns the silent-check policy: known-update cache (`Update.PendingRelease`, valid 24 h, served with no request), **1 h failure backoff**, **30 min success cooldown**, and "skip this version". Throttled silent checks return `Skipped` and issue no request; a manual check never reads or writes the throttle keys (it may refresh the result cache). Static `HttpClient` (needs `User-Agent` + `Accept: application/vnd.github+json`), ~10 s timeout, returns a result object instead of throwing. `IsInstalledBuild` delegates to `InstalledBuildInfo`. |
...
- **A known update is remembered, and while it is remembered no request is made at all**: a silent check first reads the `Update.PendingRelease` cache (the parsed `UpdateReleaseInfo`, JSON, `[JsonIgnore]` on the derived `HasInstaller`) and returns it as `Available` **without touching the network**. "There is an update" cannot become false on its own, so re-asking GitHub only burns the anonymous quota while the user has not upgraded — and it is what makes 稍后 (the dialog's *close* button, which writes no state at all) survive a restart immediately instead of waiting out a throttle window. The cache is invalidated in exactly three places, all inside `GetPendingUpdateAsync` / `SkipVersionAsync`: `SkipVersionAsync` (otherwise the next silent check would serve the just-skipped version straight back), a cached version that is no longer newer than `VersionInfo.Version` (this is what makes the post-auto-update relaunch stop nagging), and `PendingUpdateMaxAgeHours` (**24 h**, measured from the `Update.PendingReleaseUtc` write time — so a withdrawn or superseded release is corrected within a day instead of being advertised forever). An expired or unreadable cache is **deleted** and treated as absent: it is an optimisation, never a reason to miss an update, and a long-offline machine therefore stops re-showing an update it can no longer verify.
- **Both throttle windows apply only when no pending update is known**: a silent check is skipped while `Update.SilentSuccessUtc` is less than **30 minutes** old, and while `Update.SilentFailureUtc` is less than **1 hour** old. History, because every direction here has already produced a real bug report: v2.1.2 removed the pre-v2.1.2 "cached for 24 h after the last **successful** check" rule, which had meant a Release published minutes after a check stayed invisible for up to a day (reproduced against v2.0.4 — three launches in a row never reported v2.1.1). The replacement, "success leaves no trace, so every launch checks once", turned the launch count straight into a request count, and against the anonymous 60/hour cap (counted per **public IP**, so NAT/proxy neighbours share it) a few dozen relaunches in an hour produced genuine 403s — reported as `更新检查过于频繁（GitHub 接口限流）`. The success window is therefore now measured in **minutes**: ≤2 silent requests per hour per machine (1/30 of the cap), worst-case detection delay 30 min instead of a day, and the manual path still immediate. Do not widen it back to hours or days, and do not delete it.
- **Every failure branch is throttled**: HTTP error, 403/429 rate limit, timeout, `HttpRequestException`, `JsonException`, unparsable tag and unknown exceptions all funnel through `FailAsync`, which — for silent checks only — arms a **1 h backoff** via `Update.SilentFailureUtc`. A successful silent check clears the mark. No failure branch may bypass it: a 403 that is not remembered becomes a request storm that keeps the limit alive. The 1 h length is chosen against GitHub's own windows (primary limit resets within the hour, a secondary limit's `Retry-After` is typically a minute) — widening it only delays recovery, shortening it lets the retry land inside the blocked window.
- **`manual: true` writes no throttle key.** Manual checks always hit the network and neither read nor write `Update.SilentFailureUtc` / `Update.SilentSuccessUtc` — otherwise a manual check would push the next silent check back by a full window (the bug the user hit). Note the two keys are written only on a *silent* check, so a manual check immediately after a launch does not extend the 30-min cooldown. A manual check **does** refresh `Update.PendingRelease` when it finds a version: that is a result cache, not a schedule, and it is what lets the next launch show the update without a request.
...
- Settings keys (all **strings**, because `ISettingsService` only has `int`/`string` overloads — do not extend that interface for this):
  ...
  - `Update.SilentSuccessUtc` — ISO 8601 round-trip timestamp of the last **successful** silent check; a silent check within `SilentSuccessCooldownMinutes` (30) of it is skipped. Written only by the silent path.
  - `Update.PendingRelease` — JSON of the `UpdateReleaseInfo` found by the last check that saw a newer version. Served to silent checks with **no request**; dropped by `SkipVersionAsync`, by the on-read "still newer than the current build?" test, and once it is older than 24 h. It is the only update key a `manual: true` check may write.
  - `Update.PendingReleaseUtc` — when that cache was written. Missing or unparsable counts as **expired**, so it fails toward one extra request rather than toward trusting a cache of unknown age. Written together with `Update.PendingRelease` (both go through the settings service's 500 ms debounce, so it is still one file write).
  - `Update.LastCheckUtc` — **removed in v2.1.2** (it held the last *successful* check time with a 24 h window). Nothing may read it again; `UpdateService` deletes the stale key once per session, best-effort.
  - `Update.SkippedVersion` — the version the user chose to skip (silent checks stop nagging; manual checks still report it).
- Both timestamps are loaded lazily **once per session** (`EnsureThrottleStateLoadedAsync`) and are treated as absent when missing or unparsable — fail open, so a broken timestamp costs one extra request rather than silently disabling update checks. Writes happen at most ~2/hour, and only on the silent path.
- Runtime cadence lives in `MainWindow`, not in the service: `StartRuntimeUpdateCheckTimer()` (first activation) arms a repeating `DispatcherQueueTimer` (`RuntimeUpdateCheckInterval`, 24 h) whose tick reuses `RunSilentUpdateCheckAsync(isStartup: false)`, and the window's `Closed` handler stops it. Keep the timer on the UI thread — a hit has to raise a `ContentDialog` and must share the single `_isUpdateDialogOpen` guard with the manual path. Keep the service ignorant of *when* it is called: whether a call ends up on the network, on the cached result, or throttled is the service's business alone. A runtime tick with a pending update therefore re-prompts in place from the cache, with no request and no 24 h wait.
